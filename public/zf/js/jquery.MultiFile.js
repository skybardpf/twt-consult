...
/*# AVOID COLLISIONS #*/
 ...
 $.extend($.MultiFile, {
  options: {
   ...
   STRING: {
    remove:'<a href="#" class="mfiles_delete" /></a>',
    denied:'Вы не можете загружать файлы $ext .\n Попробуйте снова...',
    file:'$file',
    selected:'Выбранный файл: $file',
    duplicate:'Этот файл уже присутствует в списке:\n$file'
   }
  }
 });
     ...
     if(this._MultiFile) return; this._MultiFile = true;
       ...
       MF.addToList = function( slave ){
        ...
        var
         ...
         a = $('<span class="mfiles" title="'+MF.STRING.selected.replace('$file', v)+'">'+MF.STRING.file.replace('$file', v.match(/[^\/\\]+$/gi)[0])+'</span>'),
