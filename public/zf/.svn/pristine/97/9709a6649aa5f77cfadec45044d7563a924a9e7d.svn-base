...
require_once $root.$_GET['apppath'].'/conf/assetmanager.conf.php';
require_once $root.'zf/core/lang.class.php';

$upload = $root.trim($sBaseVirtual0, '/');
if (!is_dir($root.$sBaseFVirtual0)) {
    mkdir($root.$sBaseFVirtual0, 0777);
}
...
$sBaseRoot2="";
$sBaseRoot3="";
$sBaseRoot0=preg_replace('#'.$sBaseVirtual0.'#',"",$sBase0); //output: "c:/inetpub/wwwroot"
if($sBase1!="")$sBaseRoot1=preg_replace('#'.$sBaseVirtual1.'#',"",$sBase1);
if($sBase2!="")$sBaseRoot2=preg_replace('#'.$sBaseVirtual2.'#',"",$sBase2);
if($sBase3!="")$sBaseRoot3=preg_replace('#'.$sBaseVirtual3.'#',"",$sBase3);
...
$MaxFileSize = 3000000;
$AllowedTypes = "gif|jpg|jpeg|png|wma|wmv|swf|rtf|doc|docx|xls|xlsx|zip|rar|pdf|txt";
//$AllowedTypes = "*";
...
if(isset($_FILES["File1"]))
  ...
  $name = lang::transliterate($_FILES['File1']['name']);
  ...
  else if (move_uploaded_file($_FILES['File1']['tmp_name'], $currFolder. "/" . $name))
	{
    
	$sMsg = "";
	$sUploadedFile=$name;
	@chmod($currFolder."/".$name, 0644);
	}
  else
	{
	$sMsg = "Upload failed.";
...
if($sBase0!="")
  {
  if(strtolower($currFolder)!=preg_replace('#'.strtolower($sBase0).'#',"",strtolower($currFolder)) AND $bReadOnly0==true) $bWriteFolderAdmin=true;
  }
if($sBase1!="")
  {
  if(strtolower($currFolder)!=preg_replace('#'.strtolower($sBase1).'#',"",strtolower($currFolder)) AND $bReadOnly1==true) $bWriteFolderAdmin=true;
  }
if($sBase2!="")
  {
  if(strtolower($currFolder)!=preg_replace('#'.strtolower($sBase2).'#',"",strtolower($currFolder)) AND $bReadOnly2==true) $bWriteFolderAdmin=true;
  }
if($sBase3!="")
  {
  if(strtolower($currFolder)!=preg_replace('#'.strtolower($sBase3).'#',"",strtolower($currFolder)) AND $bReadOnly3==true) $bWriteFolderAdmin=true;
  }
$sFolderAdmin="";
...
Function recursive($sPath,$sPath_base,$sName)
  ...
  while($sItem=readdir($oItem))
	...
	if($sItem=="."||$sItem=="..")
	  ...
	  $sDisplayed=preg_replace('#'.$sBase0.'#',"",$sCurrent);
	  if($sBase1<>"") $sDisplayed=preg_replace('#'.$sBase1.'#',"",$sDisplayed);
	  if($sBase2<>"") $sDisplayed=preg_replace('#'.$sBase2.'#',"",$sDisplayed);
	  if($sBase3<>"") $sDisplayed=preg_replace('#'.$sBase3.'#',"",$sDisplayed);
	  ...
	  if(!$fIsDirectory)
		...
		if($bDisplay)
		  ...
		  if($sBaseRoot0=="") {
			...
			$sCurrent_virtual=preg_replace('#'.$sBaseRoot0.'#',"",$sCurrent);
		  }
		  if($sBaseRoot1!="")$sCurrent_virtual=preg_replace('#'.$sBaseRoot1.'#',"",$sCurrent_virtual);
		  if($sBaseRoot2!="")$sCurrent_virtual=preg_replace('#'.$sBaseRoot2.'#',"",$sCurrent_virtual);
		  if($sBaseRoot3!="")$sCurrent_virtual=preg_replace('#'.$sBaseRoot3.'#',"",$sCurrent_virtual);
...
function newFolder()
  ...
  if(navigator.appName.indexOf('Microsoft')!=-1)
	window.showModalDialog("foldernew.php?apppath=<?php echo $_GET['apppath']?>",window,"dialogWidth:250px;dialogHeight:192px;edge:Raised;center:Yes;help:No;resizable:No;");
  else
	modalDialogShow("foldernew.php?apppath=<?php echo $_GET['apppath']?>", 250, 150);
  }
function deleteFolder()
  ...
  if(navigator.appName.indexOf('Microsoft')!=-1)
	window.showModalDialog("folderdel.php?apppath=<?php echo $_GET['apppath']?>",window,"dialogWidth:250px;dialogHeight:192px;edge:Raised;center:Yes;help:No;resizable:No;");
  else
	modalDialogShow("folderdel.php?apppath=<?php echo $_GET['apppath']?>", 250, 150);
  }
