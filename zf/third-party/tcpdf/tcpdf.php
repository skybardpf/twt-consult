...
// Johannes Gьntert for JavaScript support.
...
// Esteban Joлl Marнn for OpenType font conversion.
...
if (!class_exists('TCPDF', false)) {
	 ...
	 */
	define('PDF_PRODUCER', 'Street-realty');
	...
	*/
	class TCPDF {
         ...
         * @access protected
         */
        protected $foote_phone = '(495) 258-52-23';

		/**
		 * @var Footer site
		 * @access protected
		 */
		protected $footer_site = 'http://www.streetrealty.ru/';
         ...
         * @access public
         */
        public function setHeaderData($header_title) {
		...
		/**
	 	 * Set footer data. Надо переписывать для каждого pdf
		 * @param string $ln footer image logo
		 * @param string $lw footer image logo width in mm
		 * @param string $ht string to print as title on document footer
		 * @param string $hs string to print on document footer
		 * @access public
		 */
		public function setFooterData($phone='', $site='') {
			$this->footer_phone = $phone;
			$this->footer_site  = $site;
		}

		/**
	 	 * Returns footer data: Надо переписывать для каждого pdf
	 	 * <ul><li>$ret['logo'] = logo image</li><li>$ret['logo_width'] = width of the image logo in user units</li><li>$ret['title'] = footer title</li><li>$ret['string'] = footer description string</li></ul>
		 * @return array()
		 * @access public
		 * @since 4.0.012 (2008-07-24)
		 */
		public function getFooterData() {
			$ret = array();
			$ret['phone'] = $this->footer_phone;
			$ret['site']  = $this->footer_site;
			return $ret;
		}
		...
		/**
	 	 * Set footer margin.  Надо переписывать для каждого pdf
        ...
        /**
        * Эту функцию надо переписать, отнаследовавшись от этого класса!
        * 
        */
        public function Header()
        {
            ...
            // Logo
            $image_file = K_HEADER_IMAGE_WHITE;
            $this->Image($image_file, 160, 1, 0, 0, 'JPEG', '', 'T', false, 600, '', false, false, 0, false, false, false);
            
            // print an ending header line
            $this->Line(0, PDF_MARGIN_TOP, 240, PDF_MARGIN_TOP, array('width' => 0.85 / $this->getScaleFactor(), 'cap' => 'round', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)));
            $this->SetY( 32 );
            if ($this->getRTL()) {
        ...
        */
        public function Footer() 
        {
            ...
            $this->Line(
                ...
                240, 
                $cur_y, 
                array('width' => 0.85 / $this->getScaleFactor(), 'cap' => 'round', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0))
            );
            $this->SetFont('dejavusans', 'B', 16);
                        
            $this->Cell(130, 15, $footerdata['phone'], 0, false, 'L', 0, '', 0, false, 'C', 'B');
            $this->Cell(30, 15, $footerdata['site'], 0, false, 'R', 0, '', 0, false, 'C', 'B');
        }
		 ...
		 * @author Alexander Escalona Fernбndez, Nicola Asuni
		 * @access public
		 * @since 4.5.011
		 */
		 ...
		 * @param float $lineWidth Line width, if empty or <= 0, extended up to current pdf line width (default '').
		 * @return float Return the minimal height needed for multicell method for printing the $txt param.
		 * @author Nicola Asuni, Alexander Escalona Fernбndez
		 * @access public
		 */
		...
		/**
		 * Append a cubic Bйzier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x2, y2) as the Bйzier control points.
		...
		/**
		 * Append a cubic Bйzier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using the current point and (x2, y2) as the Bйzier control points.
		 * The new current point shall be (x3, y3).
		 * @param float $x2 Abscissa of control point 2.
		 * @param float $y2 Ordinate of control point 2.
		 * @param float $x3 Abscissa of end point.
		...
		/**
		 * Append a cubic Bйzier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x3, y3) as the Bйzier control points.
		 * The new current point shall be (x3, y3).
		 * @param float $x1 Abscissa of control point 1.
		 * @param float $y1 Ordinate of control point 1.
		 * @param float $x2 Abscissa of control point 2.
		 ...
		 * @access public
		 * @author Johannes Gьntert, Nicola Asuni
		 * @since 2.1.002 (2008-02-12)
		 */
		 ...
		 * @author Andreas Wьrmser, Nicola Asuni
		 * @since 3.1.000 (2008-06-09)
		 * @access public
		 */
		...
		protected function SVGPath($d, $style='') {
			...
			foreach ($paths as $key => $val) {
				...
				switch (strtoupper($cmd)) {
					...
					case 'Q': { // quadratic Bйzier curveto
						foreach ($params as $ck => $cp) {
							$params[$ck] = $cp;
							if ((($ck + 1) % 4) == 0) {
					...
					case 'T': { // shorthand/smooth quadratic Bйzier curveto
						foreach ($params as $ck => $cp) {
							$params[$ck] = $cp;
							if (($ck % 2) != 0) {
