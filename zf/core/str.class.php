<?php
/**
 * This file contains class for dealing with multibyte strings.
 *
 * @version 1.0, SVN: $Id: str.class.php 27 2009-09-01 22:32:28Z zavter $
...
/**
 * Class for dealing with multibyte strings.
 *
 * @category Framework
 * @package zFramework
 * @subpackage Core
...
class str
{
	static public function replace($patt, $repl, $subject)	
	{
		if (!is_array($patt)) return strings::_replace($patt, $repl, $subject);
		$ret = $subject;
		for ($i = 0; $i < count($patt); $i++) {
			$ret = strings::replace($patt[$i], is_array($repl) ? $repl[$i] : $repl, $subject);
		}
		return $ret;
	}
	
	static private function _replace($pattStr, $replStr, $subject)
	{
		return preg_replace("/".$pattStr."/u", $replStr, $subject);
	}
	
	static function utf8_substr($str, $offset, $length = null)
	{
		...
		if (! is_array($a = str::utf8_str_split($str))) return false;
		if ($length !== null) $a = array_slice($a, $offset, $length);
		else                  $a = array_slice($a, $offset);
		return implode('', $a);
	}
	
	static function utf8_str_split(/*string*/ $string, /*int*/ $length = null)
	{
	...
	static function utf8_ucfirst($s, $is_other_to_lowercase = true)
	{
		if ($s === '' or ! is_string($s)) return $s;
		if (preg_match('/^(.)(.*)$/us', $s, $m) === false) return false;
		return str::utf8_uppercase($m[1]) . ($is_other_to_lowercase ? str::utf8_lowercase($m[2]) : $m[2]);
	}
...
***/
	...
	{
		return str::utf8_convert_case($s, CASE_LOWER);
	}

	static function utf8_uppercase($s)
	{
		return str::utf8_convert_case($s, CASE_UPPER);
	}
}
?>