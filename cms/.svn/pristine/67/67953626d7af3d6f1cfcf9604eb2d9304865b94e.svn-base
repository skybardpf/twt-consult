...
<br><br>
<b>Привязать к дилеру:</b> <br>
<select name="dealer">
<option value="">нет дилера</option>
{foreach from=$dealers key=key item=dealer}
<option value="{$dealer.id}"{if $dealer.id == $data.dealer_id} selected="selected"{/if}>{$dealer.name}</option>
{/foreach}
</select>
<br>
...
{foreach from=$data.state_log item=log}
  <tr class="{if $smarty.foreach.list.index % 2 eq 1}odd{/if}">
	<td class="pos"></td>
    <td>{$log.cDate}</td>
