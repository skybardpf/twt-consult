    ...
    {foreach from=$list_actions.after item=action}
        <table class="after_actions">
            <tr>
                ...
                <td class="center">
                <a href="{$root_url}{$action.link|replace:"[id]":$row.id|replace:"[pid]":$request.pid|replace:'[link]':$request.link|replace:'[type]':$request.type|replace:'[model]':$request.model}/" title="{$action.title}">{$action.title}</a>
                </td>
                <td class="right">
                </td>
            </tr>
...
{$beforeListContent}
{if $items}
{if $list_actions.multiple or $list_actions.multy_change}<form name="multiple" action="" method="post">{/if}
<table style="width:100%;" class="pages listTable">
    <tr class="pages-head">
        {if $list_actions.multiple or $list_actions.multy_change}<th><input type="checkbox" name='' id="multiple_all"></th>{/if}
    ...
    <tr>
        {if $list_actions.multiple or $list_actions.multy_change}<th></th>{/if}
    ...
    <tr class="m-item-center" {if $smarty.foreach.list.iteration%2==0}style="background-color:#e4e4e4;"{/if} id="row_{$row.id}">
        {if $list_actions.multiple or $list_actions.multy_change}<td><input type="checkbox" name='multiple_items[]' class="multiple" value={$row.id}></td>{/if}
        <td class="pos">
            {if $usePos}
        ...
        {foreach from=$titles item=columnTitle key=key name=items}
            <td>
                ...
                {if !empty($list_actions.inline.$key.title)}
                    ...
                    {/if}
                    <a{if $aClass} class="{$aClass}"{/if} title="{$aTitle}" href="{$root_url}{$list_actions.inline.$key.link|replace:"[id]":$row.id|replace:"[object_id]":$row.object_id|replace:"[rowtype]":$row.type|replace:"[user_id]":$row.user_id|replace:"[pid]":$request.pid|replace:'[model]':$request.model|replace:'[type]':$request.type}/">
                {/if}
                
                {if $row.$key == ''}
                	— 
                {elseif $fields.$key.values && !is_array($row.$key)}
                ...
                {elseif $fields.$key.field_name == 'image'}
                	{*11*}
                	{if is_array($row.$key)}
                		{assign var=title_field_name value=$fields.$key.field_name}
                		{*{$field_value.$title_field_name.0}*}
                        {foreach from=$row.$key key=k1 item=field_value name=field_values}
                        	{if $k1 == 0}
                        		<img height='40' src="{$field_value.$title_field_name|replace:'[dir]':'big'}">
                        	{/if}
                        {foreachelse}
                ...
                {elseif is_array($row.$key)}
                    {if is_array($fields.$key.field_name)}
                        {foreach from=$row.$key item=field_value name=field_values}
                            {assign var=field_name value=''}
                            {foreach from=$fields.$key.field_name item=field_val}
                                {$field_value.$field_val}
                            {/foreach}
                            {if !$smarty.foreach.field_values.last}{if $fields.$key.delim}{$fields.$key.delim}{else}, {/if}{/if}
                        {foreachelse}
                            {$fields.$key.empty|default:"не задано"}
                        {/foreach}
                    {else}
                        {assign var=title_field_name value=$fields.$key.field_name}
                        {foreach from=$row.$key item=field_value name=field_values} {*7*}
                            {$field_value.title|default:$field_value.name|default:$field_value.$title_field_name}
                            {if !$smarty.foreach.field_values.last}
                            	{if $fields.$key.delim}{$fields.$key.delim}
                            	{else},
                            	{/if}
                            {/if}
                        {foreachelse}
                ...
                {elseif $fields.$key.type eq 'date'}
                    {if $row.$key != null and $row.$key != '0000-00-00'}
                ...
                {elseif $fields.$key.type eq 'datetime'}
                	...
                	{if $row.$key == null or $row.$key == '0000-00-00 00:00:00'}
                		не установлено 
                	{else}
                ...
                {elseif $fields.$key.type eq 'date_boxes'}
                    {$row.$key|date_format:"%d.%m.%Y %H:%M"}
            ...
            <td class="del">
                {foreach from=$rightFromTable key=rKey item=rItem}
                    {if (strpos($rKey, 'delete') !== 0 OR $row.undeletable ne 'yes')}
                        ...
                        <a{if $rItem.target} target="{$rItem.target}"{/if}
                            ...
                            title="{$rItem.title}"><img src="/public/cms/img/icons/{$rItem.icon}.png" alt="{$rItem.title}" /></a>
                        {/check_cond}
                    {/if}
                {/foreach}
            </th>
